...
#import k2 #-! not essential - https://k2-fsa.github.io/k2/installation/from_wheels.html#
import torch
...
def SwooshLForwardAndDeriv(x: Tensor):
    # https://github.com/k2-fsa/k2/blob/d4f4e82f6aaee7457b088934736abf3e2ca2b066/k2/python/csrc/torch/v2/autograd/swoosh.cu#L320
    zero = torch.tensor(0.0, dtype=x.dtype, device=x.device)
    return logaddexp(zero, x - 4.0) - 0.08 * x - 0.035, torch.sigmoid(x - 4.0) - 0.08


def SwooshRForwardAndDeriv(x: Tensor):
    zero = torch.tensor(0.0, dtype=x.dtype, device=x.device)
    return logaddexp(zero, x - 1.0) - 0.08 * x - 0.313261687, torch.sigmoid(x - 1.0) - 0.08
...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        forward_activation_dict = {
            "SwooshL": SwooshLForward, #k2.swoosh_l_forward,
            "SwooshR": SwooshRForward, #k2.swoosh_r_forward,
        }
    ...
    def backward(ctx, ans_grad: Tensor):
        ...
        forward_and_deriv_activation_dict = {
            "SwooshL": SwooshLForwardAndDeriv, #k2.swoosh_l_forward_and_deriv,
            "SwooshR": SwooshRForwardAndDeriv, #k2.swoosh_r_forward_and_deriv,
        }
