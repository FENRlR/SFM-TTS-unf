...
def mel_spectrogram_torch(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False):
    ...
    if not torch.isfinite(spec).all():
        print("has NaN/Inf")
        raise RuntimeError("NaN in spec")

    if not torch.isfinite(mel_basis[fmax_dtype_device]).all():
        print("has NaN/Inf")
        raise RuntimeError(f"NaN in mel_basis[fmax_dtype_device], {fmax_dtype_device}")
